...
export async function listMessages(req:FastifyRequest, res:FastifyReply)
{
	...
	try {
		...
		if (!relation){
			return res.code(404).send({message: "No conversation found between these users"})
		}
...
export async function markConversationSeen(req:FastifyRequest, res:FastifyReply)
{
	...
	try {
		...
		if (!relation){
			...
			return res.code(404).send({message: "No conversation found between the two users"})
		}
